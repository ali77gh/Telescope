...
class HumanOnDiskAbility implements OnDiskSaveAbility<Human> {
  ...
  @override
  String toOnDiskString(Human instance) =>
...
class SaveNonBuiltInOnDiskSampleLayout extends StatefulWidget {
  ...
  @override
  State<SaveNonBuiltInOnDiskSampleLayout> createState() =>
      SaveNonBuiltInOnDiskSampleLayoutState();
}

class SaveNonBuiltInOnDiskSampleLayoutState
    extends State<SaveNonBuiltInOnDiskSampleLayout> {
  static var human = Telescope.saveOnDiskForNonBuiltInType(
      Human(187, 72), "human_for_bmi", HumanOnDiskAbility());

  // BMI = (Weight in Kilograms / (Height in Meters x Height in Meters))
  static var bmi = Telescope.dependsOn([human], () {
    return human.value.weight /
        ((human.value.height / 100) * (human.value.height / 100));
  });
